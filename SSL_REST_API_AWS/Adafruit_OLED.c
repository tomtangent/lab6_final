...
#define OCPort GPIOA2_BASE
#define OCPin 0x40

#define DCPort GPIOA1_BASE
#define DCPin 0x2
//*****************************************************************************
...
*  SPI.
*/
    GPIOPinWrite(DCPort, DCPin, 0);
    SPICSEnable(GSPI_BASE);
    GPIOPinWrite(OCPort, OCPin, 0); //cs -> low

    SPIDataPut(GSPI_BASE, c);
    SPIDataGet(GSPI_BASE, &ulDummy);

    SPICSDisable(OCPort);
    GPIOPinWrite(OCPort, OCPin, OCPin); //cs -> high
...
*  SPI.
*/
    GPIOPinWrite(DCPort, DCPin, DCPin);
    SPICSEnable(OCPort);
    GPIOPinWrite(OCPort, OCPin, 0); //cs -> low
